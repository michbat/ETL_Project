...
@click.option("--table", default="uscities_raw", help="Nom de la table cible (sans schema) ou schema.table)")
...
def main(pg_user: str, pg_pass: str, pg_host: str, pg_port: int, pg_db: str, schema: str, source: str, table: str, chunksize: int, if_exists: str) -> None:
    """Fonction main orchestrant l'ingestion des données de la table uscities_raw."""
    # Création de l'engine SQLAlchemy pour se connecter à la base de données PostgreSQL
    engine = create_engine(URL.format(user=pg_user, pw=pg_pass, host=pg_host, port=pg_port, db=pg_db))

    # Spliter le nom de la table si jamais précisé (ex: "bronze.uscities_raw" -> schema="bronze", table_only="uscities_raw")
    if "." in table:
    ...
    for df_chunk in tqdm(df_iter, desc="ingesting"):
        # Normalisation uniquement (pas de coercition de types)
